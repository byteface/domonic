...
def color(specifier: Any):
    ...
    if isinstance(specifier, Color):
        return (
            specifier.rgb()
            if not isinstance(specifier, Rgb)
            else Rgb(specifier.r, specifier.g, specifier.b, specifier.opacity)
        )
...
class Rgb(Color):
    ...
    def clamp(self) -> "Rgb":
        return Rgb(_clampi(self.r), _clampi(self.g), _clampi(self.b), max(0.0, min(1.0, self.opacity)))
    ...
    def copy(self, **kw: Any) -> "Rgb":
        return Rgb(kw.get("r", self.r), kw.get("g", self.g), kw.get("b", self.b), kw.get("opacity", self.opacity))
...
class Hsl(Color):
    ...
    def displayable(self) -> bool:
        return (0 <= self.s <= 1 or self.s != self.s) and 0 <= self.l <= 1 and 0 <= self.opacity <= 1
    ...
    def copy(self, **kw: Any) -> "Hsl":
        return Hsl(kw.get("h", self.h), kw.get("s", self.s), kw.get("l", self.l), kw.get("opacity", self.opacity))
...
class Lab(Color):
    ...
    def copy(self, **kw: Any) -> "Lab":
        return Lab(kw.get("l", self.l), kw.get("a", self.a), kw.get("b", self.b), kw.get("opacity", self.opacity))
...
class Hcl(Color):
    ...
    def copy(self, **kw: Any) -> "Hcl":
        return Hcl(kw.get("h", self.h), kw.get("c", self.c), kw.get("l", self.l), kw.get("opacity", self.opacity))
...
class Cubehelix(Color):
    ...
    def copy(self, **kw: Any) -> "Cubehelix":
        return Cubehelix(kw.get("h", self.h), kw.get("s", self.s), kw.get("l", self.l), kw.get("opacity", self.opacity))
