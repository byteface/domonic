...
class tile():
    ...
    def size(_, *args):
        return len(args) ? (x0 = y0 = 0, x1 = +_[0], y1 = +_[1], tile) : [x1 - x0, y1 - y0]

    def extent(_, *args):
        return len(args) ? (x0 = +_[0][0], y0 = +_[0][1], x1 = +_[1][0], y1 = +_[1][1], tile) : [[x0, y0], [x1, y1]]

    def scale(_, *args):
        return len(args) ? (scale = typeof _ === "function" ? _ : constant(+_), tile) : scale;

    def translate(_, *args):
        return len(args) ? (translate = typeof _ === "function" ? _ : constant([+_[0], +_[1]]), tile) : translate

    def zoomDelta(_, *args):
        return (zoomDelta = +_, tile) if len(args) else zoomDelta

    def tileSize(_, *args):
        return (tileSize = +_, tile) if len(args) else tileSize

    def clamp(_, *args):
        return (clampX = clampY = !!_, tile) if len(args) else clampX and clampY

    def clampX(_, *args):
        return (clampX = !!_, tile) if len(args) else clampX

    def clampY(_, *args):
        return (clampY = !!_, tile) if len(args) else clampY
