...
class TestCase(unittest.TestCase):
    def test_odf_text_document(self):
        ...
        self.assertIn(
            'xmlns:office="urn:oasis:names:tc:opendocument:xmlns:office:1.0"', rendered
        )
    ...
    def test_odf_namespaced_globals_and_factory(self):
        self.assertEqual(
            str(odf.__dict__["table:table"]()), "<table:table></table:table>"
        )
        self.assertEqual(
            str(create_odf_element("draw:image", **{"xlink:href": "Pictures/one.png"})),
            '<draw:image xlink:href="Pictures/one.png"></draw:image>',
        )
    ...
    def test_odf_generated_constructor_pattern(self):
        ...
        self.assertEqual(
            odf_tag_aliases["office_document_content"], "office:document-content"
        )
        ...
        heading = text_h(
            "Heading", text_outline_level=2, text_style_name="Heading_20_2"
        )
        self.assertEqual(
            str(heading),
        ...
        image = create_element(
            "draw_image",
            xlink_href="Pictures/one.png",
            svg_width="4cm",
            svg_height="3cm",
        )
        ...
        manifest_doc = manifest_manifest(
            manifest_file_entry(manifest_full_path="/", manifest_media_type="text/xml")
        )
