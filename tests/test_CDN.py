...
class TestCase(unittest.TestCase):
    def test_domonic_CDN(self):
        ...
        assert (
            str(mycss)
            == '<link href="https://cdn.jsdelivr.net/npm/marx-css@5.3.0/css/marx.min.css"/>'
        )
        ...
        assert (
            CDN_IMG.PLACEHOLDER(100, 100, separator="x") == "//loremflickr.com/100x100"
        )
        assert (
            CDN_FONT.google("Open Sans")
            == "https://fonts.googleapis.com/css?family=Open+Sans"
        )
        assert (
            CDN_FONT.google(["Open Sans", "Roboto"])
