...
first call over a tree builds lookup maps on the root, later calls answer from
them, and any mutation invalidates them automatically. See the
...
domonic has several interchangeable parser backends — pick for zero
dependencies, malformed-HTML repair, or raw speed:
...
renderer at import time — no DOM is constructed per request:
